...
│   ├── サンドボックス実行環境                                      │
│   └── servers/growi/*.ts (生成コードAPI)                         │
│         │                                                       │
│         │ ファイルシステム探索 ← ★遅延読み込みはここで実現        │
│         │                                                       │
│         └── callMCPTool('getPage', {...})                       │
└─────────────────────────────────────────────────────────────────┘
                            │
...
1. **遅延読み込みは Code Interpreter 内で実現される**
   - ファイルシステムベースでツール定義を探索
   - 必要な `*.ts` ファイルだけを読み込む
   - MCP の `tools/list` は使用しない
...
| Code Interpreter MCP | コード実行・ファイル探索 | **遅延読み込みを実現** |
| 生成コード API (`servers/growi/`) | ツール定義をファイルとして提供 | 探索対象 |
