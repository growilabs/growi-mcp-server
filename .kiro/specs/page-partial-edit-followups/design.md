...
見出しの位置は行・列ではなく**文字オフセット**（`position.start.offset` / `end.offset`）で扱う。構文木がオフセットを持っているため、自前の行配列を作らずに次が得られる。

| 求めるもの | 取り方 |
| --- | --- |
| `totalLines` | `tree.position.end.line`（実測で行分割の結果と完全一致。末尾改行の有無・空本文・CRLF・裸の `\r` すべて） |
| `chars` | セクション開始オフセットから次セクション開始オフセットまでの差（末尾の改行 1 つを除く） |
| `raw` | `body.slice(start.offset, end.offset)` から ATX マーカーと閉じ列（setext なら下線行）を落とす |

これは行数の削減以上に、**M1 と同じ種類のバグが入る余地を減らす**ことが目的である。M1 は「行番号は構文木から、その行の内容は自前の行配列から」という 2 つの座標系が食い違って起きた。オフセットに寄せると、両者が出会う場所は `endLine`（構文木の行番号）と `getPageSection` の行スライスの 1 箇所だけになる。そのため `splitLines` は残し、行の定義がこのモジュールで 1 つに保たれることを担保する。

`raw` をオフセットで切り出す副次的な利点として、**`raw` が必ず本文の部分文字列になる**。行ごとに組み立て直す方式では、引用やリスト項目の内側にある複数行 setext 見出しで `"Title\nmore"` のように本文に存在しない文字列を返し、`editPage` の `oldString` として使えなかった。

**`chars` の定義変更（振る舞いの変更）**: セクションの文字数は「そのセクション自身のテキストの長さ」とし、次のセクションと区切る改行 1 つを含めない。従来は最終セクションの範囲が末尾の空行まで伸びていたため、**同じ内容でもファイルが改行で終わるかどうかで値が 1 変わっていた**（`# A\ntail` は 8、`# A\ntail\n` は 9）。新しい定義ではどちらも 8 になる。

代償として「各セクションの `chars` を足すと本文全体の長さに戻る」性質は失われる（末尾改行の 1 文字分だけ足りなくなる）。これを必要とする利用者はいない。全体の長さは `totalChars` が `body.length` そのままで返しており、部分から足す必要がないためである。`chars` を参照する内部ロジックも無い（`getPageSection` の切り詰めは `maxChars` と実際の行の長さで判定する）。

リポジトリ内の markdown 68 ファイル・見出し 859 件で旧実装と突き合わせた結果、差分は `chars` の 127 件（すべて −1）のみで、`totalLines` / `totalChars` / 見出し数 / `level` / `text` / `raw` / `startLine` / `endLine` / `preamble` はすべて一致した。127 件は「最終行まで伸びるセクション」（最後の h1・その中の最後の h2・h3…）が末尾改行を拾っていた分である。
